...
const fetchFromAPI = async () => {
    const resp = await fetch('https://api.twitch.tv/kraken/chat/emoticon_images?emotesets=0', {
        headers: {
    ...
    return data.emoticon_sets['0'].find(emoticon => emoticon.code === 'PogChamp').id;
};

// Get the PogChamp emote by sending a message in chat
// This was needed originally as the API returned the removed, original PogChamp for a while
// This needs the tmi.js library installed
...
const main = async () => {
    ...
    const id = await fetchFromAPI().catch(e => {
        console.warn(e);
